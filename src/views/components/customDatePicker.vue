<template>
    <div class="custom-datepicker" :class="{ 'my-datepicker-disabled': disabled }">
        ...
        <date-picker v-model="inputVal" format="DD.MM.YYYY" type="date" @change="$emit('change', value)"
            input-class="my-custom-date " :clearable="false" :disabled="disabled" :disabled-date="disabledDate"
            :lang="lang == 'ru' ? ru : lang == 'uz_cyrl' ? uz : uz" style="width:100%">
            <template #input>
                <b-form-input :disabled="disabled" @keyup="$emit('keyup', value)" @input="Change" :placeholder="placeholder"
                    v-mask="'##.##.####'" type="text" maxlength="10" v-model="value" class="my-custom-date" />
            </template>
        </date-picker>
    </div>
</template>
